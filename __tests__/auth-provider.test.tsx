...
const clientMock = mocked(new  AuctoritasClient({ client_id: '', domain: '' }));

describe(' AuctoritasProvider', () => {
  afterEach(() => {
    window.history.pushState({}, document.title, '/');
  });

  it('should provide the  AuctoritasProvider result', async () => {
  ...
  it('should configure an instance of the  AuctoritasClient', async () => {
    const opts = {
      clientId: 'foo',
      domain: 'bar',
      redirectUri: 'baz',
    ...
    const { waitForNextUpdate } = renderHook(() => useContext(ClientContext), {
      wrapper,
    });
    expect( AuctoritasClient).toHaveBeenCalledWith(
  ...
  it('should pass user agent to  AuctoritasClient', async () => {
    const opts = {
      clientId: 'foo',
      domain: 'bar',
    };
  ...
  it('should skip redirect callback for non  Auctoritas redirect callback handlers', async () => {
    clientMock.isAuthenticated.mockResolvedValue(true);
    window.history.pushState(
      {},
      document.title,
      '/?code=__some_non_ Auctoritas_code__&state=__test_state__'
    );
    clientMock.handleRedirectCallback.mockRejectedValue(
      new Error('__test_error__')
    );
  ...
  it('should call getAccessTokenSilently in the scope of the  Auctoritas client', async () => {
  ...
  it('should call getAccessTokenWithPopup in the scope of the  Auctoritas client', async () => {
  ...
  it('should call handleRedirectCallback in the scope of the  Auctoritas client', async () => {
